...
export default function Home() {
  ...
  return (
    <div className="min-h-screen text-zinc-100 font-sans antialiased pb-8">
      <Navbar />

      <main className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-5">
        {/* Mobile Tab Switcher */}
        <div className="flex lg:hidden items-center p-1.5 glass-pill rounded-full mb-4">
          {[
            { key: 'chat', label: 'AI Assistant', count: null },
            { key: 'tasks', label: 'Task Queue', count: pendingTasks.length },
          ].map((tab) => (
            <button
              key={tab.key}
              onClick={() => setActiveMobileTab(tab.key as any)}
              className={`flex-1 py-2 rounded-full text-xs font-bold transition-all duration-300 flex items-center justify-center gap-2 ${
            ...
            >
              ...
              {tab.count !== null && (
                <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-mono ${
                  activeMobileTab === tab.key ? 'bg-white/20 text-white' : 'bg-zinc-800 text-zinc-400'
                }`}>
                  {tab.count}
                </span>
              )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className={`lg:col-span-7 space-y-4 ${activeMobileTab === 'chat' ? 'hidden lg:block' : 'block'}`}>
            ...
            <div className="doppelrand-shell">
              <div className="doppelrand-core p-4 sm:p-5 space-y-4">
                <div className="flex items-center justify-between gap-3 flex-wrap">
                  <div>
                    ...
                    <p className="text-xs text-zinc-500 mt-0.5 font-mono">
                      Drag to reorder • Click card to inspect
                    </p>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="btn-pill-primary active:scale-95"
                    >
                      <span>{showAddForm ? 'Cancel' : 'Add Task'}</span>
                      <div className="btn-icon-pod">
                    ...
                    <button
                      onClick={fetchTasks}
                      disabled={isLoading}
                      className="p-2 text-zinc-400 hover:text-zinc-200 rounded-full glass hover:border-white/20 transition-all disabled:opacity-50 active:scale-95"
                      title="Refresh queue"
                    >
                ...
                <div className="grid grid-cols-3 gap-2.5 pt-1">
                  <div className="p-3 rounded-2xl bg-zinc-950/60 border border-white/[0.05] text-center shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
                    <div className="text-[10px] uppercase tracking-widest font-mono text-zinc-400 font-bold">Active</div>
                    <div className="text-xl font-extrabold text-indigo-400 font-mono mt-0.5">{pendingTasks.length}</div>
                  </div>
                  <div className="p-3 rounded-2xl bg-zinc-950/60 border border-white/[0.05] text-center shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
                    <div className="text-[10px] uppercase tracking-widest font-mono text-zinc-400 font-bold">Overdue</div>
                    <div className={`text-xl font-extrabold font-mono mt-0.5 ${overdueTasks.length > 0 ? 'text-red-400 animate-pulse-subtle' : 'text-zinc-600'}`}>
                      {overdueTasks.length}
                    </div>
                  </div>
                  <div className="p-3 rounded-2xl bg-zinc-950/60 border border-white/[0.05] text-center shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
                    <div className="text-[10px] uppercase tracking-widest font-mono text-zinc-400 font-bold">Done</div>
                    <div className="text-xl font-extrabold text-emerald-400 font-mono mt-0.5">{completedTasks.length}</div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between gap-2.5 pt-1 flex-wrap">
                  ...
                  <div className="relative flex-1 min-w-[160px]">
                    ...
                    <input
                      ...
                      className="w-full pl-8 pr-7 py-2 bg-zinc-950/80 border border-white/[0.08] rounded-xl text-xs text-zinc-200 placeholder-zinc-600 focus:outline-none focus:border-indigo-500/50 focus:ring-2 focus:ring-indigo-500/20 transition-all"
                    />
                    {searchQuery && (
                      <button
                  ...
                  {/* Filter Pills */}
                  <div className="flex items-center gap-1 bg-zinc-950 p-1 rounded-xl border border-white/[0.08]">
            ...
            {showAddForm && (
              <form onSubmit={handleManualAdd} className="glass-elevated rounded-3xl p-5 space-y-3.5 animate-scale-in border border-indigo-500/30 shadow-2xl">
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 bg-zinc-950 border border-white/[0.1] rounded-2xl text-sm text-zinc-100 placeholder-zinc-600 focus:outline-none focus:border-indigo-500/60 focus:ring-2 focus:ring-indigo-500/20 transition-all"
                  autoFocus
                  required
                />
                <textarea
                  ...
                  rows={2}
                  className="w-full px-4 py-2.5 bg-zinc-950 border border-white/[0.1] rounded-2xl text-sm text-zinc-100 placeholder-zinc-600 focus:outline-none focus:border-indigo-500/60 focus:ring-2 focus:ring-indigo-500/20 transition-all resize-none"
                />
                <div className="flex items-center gap-3">
                  <input
                    ...
                    className="flex-1 px-3.5 py-2 bg-zinc-950 border border-white/[0.1] rounded-xl text-xs text-zinc-300 focus:outline-none focus:border-indigo-500/60"
                  />
                  <select
                    value={newImportance}
                    onChange={(e) => setNewImportance(parseInt(e.target.value))}
                    className="px-3.5 py-2 bg-zinc-950 border border-white/[0.1] rounded-xl text-xs text-zinc-300 focus:outline-none focus:border-indigo-500/60 font-semibold"
                  >
                ...
                <button
                  ...
                  className="w-full py-2.5 bg-gradient-to-r from-indigo-600 via-purple-600 to-indigo-600 hover:from-indigo-500 hover:to-purple-500 text-white text-sm font-bold rounded-2xl transition-all disabled:opacity-40 shadow-lg shadow-indigo-500/25 active:scale-[0.99]"
                >
            ...
            {isLoading ? (
              ...
              <div className="glass rounded-3xl p-12 text-center animate-fade-up border border-white/[0.06]">
              ...
              <div className="space-y-3 max-h-[calc(100vh-21rem)] overflow-y-auto pr-1 stagger-children custom-scrollbar">
